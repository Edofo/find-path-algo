...
def traverse_all_edges(graph, start, total_edges, total_length, battery_capacity, recharge):
    ...
    while stack:
        ...
        for i, (next_node, length) in enumerate(graph[current_node]):
            ...
            # get neighbors of next node
            neighbors = graph[next_node]
            print("Neighbors of next node:", neighbors)
            # remove current node from neighbors
            neighbors = [neighbor for neighbor in neighbors if neighbor[0] != current_node]
            print("Neighbors of next node after removing current node:", neighbors)
            ...
            # if all neighbors of next node are visited, check the next neighbor of current node
            if all_visited:
                continue
